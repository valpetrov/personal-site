"""Generate Val Petrov's traditional (classic) software engineering resume as a PDF.

A straightforward, recruiter-friendly resume that leads with distributed-systems
and FinTech engineering experience — deliberately free of any "agentic / AI" framing.
Shares the same refined layout as build_resume.py but with conventional content.
"""
...
def draw_header(c):
    ...
    c.drawString(MAIN_X, y, "SENIOR SOFTWARE ENGINEER  ·  NEW YORK, NY")
    ...
    # Tagline — italic serif with gold em-style on "distributed systems"
    y -= 18
    c.setFont(SERIF_I, 11.5)
    c.setFillColor(SUBINK)
    text = "Architecting mission-critical "
    c.drawString(MAIN_X, y, text)
    w = c.stringWidth(text, SERIF_I, 11.5)
    c.setFillColor(ACCENT)
    c.drawString(MAIN_X + w, y, "distributed systems")
    w2 = c.stringWidth("distributed systems", SERIF_I, 11.5)
    c.setFillColor(SUBINK)
    c.drawString(MAIN_X + w + w2, y, ".")
...
def draw_sidebar(c):
    ...
    # ===== SKILLS =====
    y = draw_sidebar_section_title(c, y, "Technical Skills")
    ...
    skill_groups = [
        ...
        ("Data & Messaging", [
            ...
            "Hadoop / HDFS · Solr · HBase",
        ]),
        ("Platforms & Tooling", [
            ".NET · WPF · WinForms",
            "Flask · Git · Agile / Scrum",
        ]),
        ("Domain Expertise", [
            "FinTech / Wall Street",
    ...
    # ===== METRICS =====
    y = draw_sidebar_section_title(c, y, "Highlights")

    metrics = [
        ("14+", "Years building distributed systems"),
...
def draw_main(c, y):
    # ===== SUMMARY =====
    y = draw_main_section_title(c, y, "Summary")
    summary = (
        "Senior software engineer and engineering lead with 14+ years architecting mission-critical, "
        "high-throughput distributed systems for top-tier Wall Street firms. Deep expertise across "
        "real-time data pipelines, event-driven architecture, trading and portfolio platforms, and "
        "reference-data infrastructure. Proven track record leading teams, owning systems end-to-end, "
    ...
    experiences = [
        {
            ...
            "tech": ["Murex", "C#", ".NET", "Python", "SQL Server", "Sybase", "Kafka", "Databricks"],
        },
        {
            ...
            "tech": ["Python", "SQL Server", "Databricks", "Git"],
        },
        {
    ...
    for exp in experiences:
        ...
        if y < MARGIN_B + 110:
            ...
            c.setFillColor(INK)
            c.setFont(SERIF_B, 14)
            c.drawString(MARGIN_L, sy, "Senior Engineer")
            sy -= 14
            c.setFillColor(SUBINK)
            c.setFont(SANS, 8)
...
def draw_footer(c):
    ...
    c.setFillColor(MUTED)
    c.setFont(MONO, 6.5)
    c.drawString(MARGIN_L, y + 6, "VAL PETROV  ·  SENIOR SOFTWARE ENGINEER  ·  NEW YORK, NY")
...
def build(out_path="val-petrov-traditional-resume.pdf"):
    c = canvas.Canvas(out_path, pagesize=LETTER)
    c.setTitle("Val Petrov — Resume")
    c.setAuthor("Val Petrov")
    c.setSubject("Resume — Senior Software Engineer")
